...
from src.data import storage, vocabulary
from src.data.preprocessing import PreprocessingBase
from constants import UNK, VOCAB_POS_TAGGING_PATH, OUTPUT_PREPROCESSED_DATA, VOCAB_WORDS_PATH

logger = logging_module.get_logger(__name__)


class PreprocessingEditNTS(PreprocessingBase):

    def __init__(self, **kwargs):

        self.original_text_path = kwargs.get('original_text_path')
        self.simple_text_path = kwargs.get('simple_text_path')
    ...
    def pipeline(self):
        ...
        df = pd.DataFrame(
            {
                "comp_tokens": self.tokenize(original_txts),
                "simp_tokens": self.tokenize(simple_txts),
            }
        )
    ...
    def _add_pos(self, df: DataFrame) -> DataFrame:
    ...
    def _add_edits(self, df: DataFrame) -> DataFrame:
    ...
    def _editnet_data_to_editnetID(self, df: pd.DataFrame):
        ...
        def prepare_example(example, vocab):
            ...
            """
            comp_id = np.array(
                [vocab.w2i[i] if i in vocab.w2i.keys() else vocab.w2i[UNK] for i in example['comp_tokens']])
            simp_id = np.array(
                [vocab.w2i[i] if i in vocab.w2i.keys() else vocab.w2i[UNK] for i in example['simp_tokens']])
            edit_id = np.array(
                [vocab.w2i[i] if i in vocab.w2i.keys() else vocab.w2i[UNK] for i in example['edit_labels']])
    ...
    def _sent2edit(self, sent1: List[str], sent2: List[str]) -> List[str]:

        dp = self._edit_distance(sent1, sent2)
        edits = []
        pos = []
    ...
    def _edit_distance(self, sent1: List[str], sent2: List[str], max_id: int = 4999) -> List[List[int]]:
